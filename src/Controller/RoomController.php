...
class RoomController
{
    ...
    public function index(): void
    {
        ...
        $page = $_GET['page'] ?? 'home';
        $allowedPages = ['home', 'rooms', 'booking', 'mybookings', 'report'];
        ...
        require_once __DIR__ . '/../View/student/rooms/index.php';
    }
    ...
    private function handleReport(int $userId): array
    {
        ...
        if ($errors === [] && !$this->roomRepo->deviceExists((int) $deviceId)) {
    ...
    private function handleCancelBooking(int $userId): array
    {
        ...
        if (!$canCancel) {
            $errors['cancel'] = 'Yêu cầu đặt phòng này không thể hủy.';
            return [$errors, $success];
        }

        $this->roomRepo->cancelBooking($bookingId, $userId);
        $success = 'Đã hủy yêu cầu đặt phòng.';
